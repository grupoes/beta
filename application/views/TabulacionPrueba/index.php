<form id="formulario" >
  <div  class=" panel panel-flat "  >
    <div class="panel-heading" id="cuerpo">
       <div class="row"><button type="button" id="migrar" class="btn btn-success" onclick="ver_data()">migrar</button></div>
    </div>  
    <div class="panel-body">
      <div class="row">
        <center><h3>Configuracion de la tabulacion xx</h3></center>
        <div class="col-md-3">
...
<div class="row">
  <br><br>
  <center><button class="btn btn-primary" type="button" id="generar">Generar Variables</button><button class="btn btn-danger">Cancelar</button></center>
</div>
</div>
</div>

<div  class=" panel panel-flat " id="primeravariable" style='display:none;' >
...
<div  id="segunvaria" style='display:none;'>
  <div  class=" panel panel-flat"  >
    ...
    <div class="panel-body">
      ...
      <div class="row">
        ...
        <div class="col-lg-6  "><input type="number" value="0" class="form-control" name="porcentajevar" id="porcentajevar" ></div> 
        </div>
      </div>
    </div>
  </div>
...
<div  class=" panel panel-flat " id="dimensionesrelacion" style='display:none;' >
  <div class="panel-heading" id="deta"></div>  
  <div class="panel-body">
    <div class="row">
...
<div  class=" panel panel-flat " id="detalle" style='display:none;' >
          ...
          <div class="ec_calculator_gen clearfix">
            ...
            <div id="overlayLoader">
              ...
              <div id="dispCalcConts">
                <table>
                  <tr><td align=center> <b>Para Calcular coeficiente de correlación:</b></td></tr>
                  <tr><td>
                    <table id=a>
                      <tbody id="prueba">
                      </tbody>
          ...
          <div class="ec_calculator_gen clearfix">
            ...
            <div id="overlayLoader">
              ...
              <div id="dispCalcConts">
                ...
                <table>
                  <tr><td align=center> <b>Cuadro Dimension V1:</b></td></tr>
                  <tr><td>
                    <table id=b>
                      <tbody id="dimension1">
                      </tbody>
        ...
        <div class="col-lg-4"> 
          <div class="ec_calculator_gen clearfix">
            ...
            <div id="overlayLoader">
              ...
              <div id="dispCalcConts">
                <table>
                  <tr><td align=center> <b>Cuadro Dimension V2:</b></td></tr>
                  <tr><td>
                    <table id=c>
                      <tbody id="dimension2">
                      </tbody>
            ...
            <div class="col-lg-4"><br><center><input id="coeficienterelacion" value="0.00" type="text" readonly></center></div><br><br>
            <div><br><center><button type="submit"   class="btn btn-primary">Generar Tabulacion</button></center></div>
          </div>   
        </div>
      </div>
      <!--  -->
...
$(window).scroll(function(){
    ...
    scroll_critical = parseInt($("#primeravariable").height()) + parseInt($("#segunvaria").height()) + parseInt($("#dimensionesrelacion").height()) +800; // VALOR DE TU DIV
...
if ($("#muestra").val() == '' || $("#muestra").val() == '' || $("#nommuestra").val() == '' || $("#escala").val() == '' || $("#respuesta").val() == '') {
  alert('Falta llenar datos, llenelos por favor!');
  return 0;
}

  if($("#variable").val() == 2){
    document.getElementById('primeravariable').style.display = 'block';
    document.getElementById('segunvaria').style.display = 'block';

  }else{
    document.getElementById('primeravariable').style.display = 'block';
    document.getElementById('segunvaria').style.display = 'none';
  }
  document.getElementById('dimensionesrelacion').style.display = 'block';
...
if ($("#variable").val() == 2) {
html1='<div><br><center><button type="submit"  onclick="generartabulacion()" class="btn btn-primary">Generar Tabulacion</button><button type="button" onclick="generarmatrices()"  class="btn btn-primary">Generar Matrices</button></center></div>';
}else{
html1='<div><br><center><button type="submit"  onclick="generartabulacion()" class="btn btn-primary">Generar Tabulacion</button></center></div>'; 
}
...
function generarmatrices(){
  var matrices = matriz1();
  envio = 2;
  if($("#variable").val() == 2){
   var matrices2 = matriz2();
   calcular_porcentaje();
   add();
   dimensiones();
  }
...
function add(){ 
  html="";
  html+="<tr>"
  html+="<td width=20%>V1 N° Encontrado</td>";
  html+="<td width=20%>V1 N° Escala</td>";
  html+="<td width=20%></td>" ;
  html+="<td width=20%>V2 N° Escala</td>";
  html+="<td width=10%>V2 N° Encontrado</td>";
  html+="<td width=10%> </td>";
  html+="</tr>";
  var mues1 =parseInt($("#item").val()) + 1;
  var mues2 =parseInt($("#itemv2").val()) + 1;
  for (var i = 0; i < $("#muestra").val(); i++) {
  var yx = "yescala"+i;
  html+='<tr>';
  html+='<tr>';
  html+='<td>';
  html+=  '<div class="col-lg-12"><div class="input-group">';
  html+=  '<input id="xvalor'+i+'" value="'+excel[i][mues1]+'" type="text" class="form-control" readonly>';
  html+=  '</div></div>';
  html+='</td>';
  html+='<td>';
  html+=  '<div class="col-lg-12"><div class="input-group">';
  html+=  '<input id="xescala'+i+'" value="'+ordenalea[i]+'" type="text" class="form-control" readonly>';
  html+=  '</div></div>';
  html+='</td>';
  html+=  '<td><div class="col-lg-12"><div class="input-group"><span class="input-group-addon"><i class="icon-arrow-right16"></i></span>';
  html+=  '</div></div></td>';
  html+='<td>';
  html+=  '<div class="col-lg-12"><div class="input-group">';
  html+=  '<input id="yescala'+i+'" value="'+ordenaleayuda[i]+'" onkeyup="checnum('+ordenaleayuda[i]+','+i+')" type="text" class="form-control">';
  html+=  '</div></div>';
  html+='</td>';
  html+='<td>';
  html+=  '<div class="col-lg-12"><div class="input-group">';
  html+=  '<input id="yvalor'+i+'" value="'+excel2[i][mues2]+'"  type="text" class="form-control" readonly>';
  html+=  '</div></div>';
  html+='</td>';
  html+='<td>';
  html+=  '<div class="col-lg-12"><div id="color'+i+'" class="input-group">';
  if (ordenalea[i] == ordenaleayuda[i]) {
...
function dimensiones(){
  ...
  html="";
  html+="<tr>";
  html+="<td width=10%>N°</td>"; 
  html+="<td width=20%>Descripcion</td>";
  html+="<td width=20%>Desde</td>";
  html+="<td width=20%>Hasta</td>" ;
  html+="<td width=20%>Porcentaje</td>";
  ...
  var cantidad = [];
  var nombreescala = [];
  var maximo=parseFloat($("#respuesta").val())*parseFloat($("#item").val());
  var minimo=parseFloat($("#item").val());
  var rango=maximo-minimo;
  var amplitud=Math.round(rango/parseFloat($("#escala").val()));
  $('input[id="nombre_escala[]').map(function ( n, i) {
    j = n;
    desde[n] = minimo;
    hasta[n] = minimo+(amplitud-1);
    minimo = minimo+amplitud;
    nombreescala[n] =$(this).val();
  }).get();
  $('input[name="porcentaje[]').map(function ( n, i) {
    porcentaje[n] =$(this).val();
  }).get();
  $('input[name="cantidad[]').map(function ( n, i) {
    cantidad[n] =$(this).val();
  }).get();
  hasta[j] = maximo;
  for (var i = 0; i < $("#escala").val(); i++) {
    html+='<tr>';
    html+='<td>';
    html+=  '<div class="col-lg-0"><div class="input-group">';
    html+=  '<input id="" value="'+i+'" type="text" class="form-control" readonly>';
    html+=  '</div></div>';
    html+='</td>';
    html+='<td>';
    html+=  '<div class="col-lg-12"><div class="input-group">';
    html+=  '<input id="" value="'+nombreescala[i]+'" type="text" class="form-control" readonly>';
    html+=  '</div></div>';
    html+='</td>';
    html+='<td>';
    html+=  '<div class="col-lg-12"><div class="input-group">';
    html+=  '<input id="" value="'+desde[i]+'" type="text" class="form-control" readonly>';
    html+=  '</div></div>';
    html+='</td>';

    html+='<td>';
    html+=  '<div class="col-lg-12"><div class="input-group">';
    html+=  '<input id="" value="'+hasta[i]+'" type="text" class="form-control" readonly>';
    html+=  '</div></div>';
    html+='</td>';

    html+='<td>';
    html+=  '<div class="col-lg-12"><div class="input-group">';
    html+=  '<input id="" value="'+porcentaje[i]+'"  type="text" class="form-control" readonly>';
    html+=  '</div></div>';
    html+='</td>';
    html+='<td>';
    html+=  '<div class="col-lg-12"><div class="input-group">';
    html+=  '<input id="" value="'+cantidad[i]+'"  type="text" class="form-control" readonly>';
    html+=  '</div></div>';
    html+='</td>';
    html+='</tr>';
  }
  $("#dimension1").empty().append(html);
  ...
  var hastav2 = [];
  var nombreescalav2 = [];
  var maximo=parseFloat($("#respuesta").val())*parseFloat($("#itemv2").val());
  var minimo=parseFloat($("#itemv2").val());
  var rango=maximo-minimo;
  var amplitud=Math.round(rango/parseFloat($("#escala").val()));
  ...
  for (var i = 0; i < $("#escala").val(); i++) {
    ...
    html+='<td>';
    html+=  '<div class="col-lg-12"><div class="input-group">';
    html+=  '<input id="" value="'+nombreescalav2[i]+'" type="text" class="form-control" readonly>';
    html+=  '</div></div>';
    html+='</td>';
    html+='<td>';
    html+=  '<div class="col-lg-12"><div class="input-group">';
    html+=  '<input id="" value="'+desdev2[i]+'" type="text" class="form-control" readonly>';
    html+=  '</div></div>';
    html+='</td>';

    html+='<td>';
    html+=  '<div class="col-lg-12"><div class="input-group">';
    html+=  '<input id="" value="'+hastav2[i]+'" type="text" class="form-control" readonly>';
    html+=  '</div></div>';
    html+='</td>';

    html+='<td>';
    html+=  '<div class="col-lg-12"><div class="input-group">';
    html+=  '<input id="" value="'+porcentajev2x[i]+'"  type="text" class="form-control" readonly>';
    html+=  '</div></div>';
    html+='</td>';
    html+='<td>';
    html+=  '<div class="col-lg-12"><div class="input-group">';
    html+=  '<input id="" value="'+cantidadv2x[i]+'"  type="text" class="form-control" readonly>';
    html+=  '</div></div>';
    html+='</td>';
    html+='</tr>';
  }
  $("#dimension2").empty().append(html);
}
function checnum(ordenale,orden){
  var escala = parseInt($("#escala").val()) - 1 ;
  var valor = $("#yescala"+orden).val();
  data=[];
  sumax = 0;
  var deficitv2= [];
  cant = $("#escala").val();
  for(var i=1; i<= $("#itemv2").val();  i++){
    deficitv2[i] = 0;
  }
  console.log(desdev2[valor]+","+hastav2[valor]);
  if( parseInt(valor) <= parseInt(escala)){
    ordenaleayuda[orden] =  valor;
    data=generar_matriz(desdev2[valor],hastav2[valor],$("#itemv2").val(),$("#respuesta").val(),deficitv2,cant,valor,0,2);
    console.log(data); 
    excel2[orden] = new Array();
    for (j=0; j <= $("#itemv2").val() ; j++) {

      if(j==0){
        excel2[orden][j]= $("#nommuestra").val()+' '+(orden+1);
      }else{
        console.log(data[j-1]);
        if(j==($("#itemv2").val()+1)){
          excel2[orden][j]=data[j-1];
        }else{
          excel2[orden][j]=data[j-1];
          sumax = parseInt(sumax) + parseInt(data[j-1]);
        }                
      }  
    }  
    excel2[orden][j]=sumax;
    console.log(excel2);
    coeficiente = parseFloat(parseFloat(calculate(excel,excel2,$("#muestra").val(),$("#item").val(),$("#itemv2").val())).toFixed(2));
    
    $("#coeficienterelacion").val("Coef.  relación "+ coeficiente);
    $("#yescala"+orden).val(valor);
    $("#yvalor"+orden).val(sumax);
    if (ordenalea[orden] == ordenaleayuda[orden]) {
    html=  '<span  class="label label-success"> = </span>';
    }else{
    html=  '<span  class="label label-danger"> != </span>';
    }
    $("#color"+orden).empty().append(html);
    dimensiones();
  }else{
    alert('La escala que incluyo es muy alta');
    $("#yescala"+orden).val(ordenale);
  }
}
...
var $mensaje = "¿Está seguro de Generar la Tabulación?";  
}
...
function calcular_porcentaje(){
  ...
  porcentajev2x = {};
  for(var i = 0; i< $("#muestra").val(); i++){
    cantidadv2x[ordenaleayuda[i]] = cantidadv2x[ordenaleayuda[i]] ? cantidadv2x[ordenaleayuda[i]] + 1 : 1;
  }
  var porcen = 0;
  for(var i = 0; i< $("#escala").val(); i++){
    porcen = parseFloat(parseInt(cantidadv2x[i]) / parseInt($("#muestra").val()))*100;
    porcentajev2x[i] = Math.round(porcen);
  }
}
